...
export const profileJp =
    ...
    さらに5年の紆余曲折を経て、現在はフリーランスとして独立。コーヒーをすすり, プロテインバーをかじり, 妻の用意する生野菜をモシャりながら VSCode で <code>go test</code> したり <code>wrangler publish -e production</code> したり <code>docker</code> の <code>compose</code> を <code>up -d</code> したりする毎日。
    `.replace(/\n/g, "<br />");
